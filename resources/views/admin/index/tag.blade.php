              ...
              <table class="table table-striped jambo_table bulk_action" >
                ...
                <tbody id="tbl_alproduct">
                  ...
                  <tr class="even pointer">
                    ...
                    <td>
                      @if ($value->status)
                        <a class="btn_status_product" href="/admin/tag/status/{{$value->id}}" rel="{{$value->id}}"><i class="success fa fa-eye" title="Hiển thị"></i></a>
                      @else
                        <a class="btn_status_product" href="/admin/tag/status/{{$value->id}}" rel="{{$value->id}}"><i class="success fa fa-eye-slash" title="Ẩn"></i></a>
                      @endif
                    </td>
                    <td>{{$value->title}}</td>
                    <td><input type="text" value="{{$value->link}}"></td>
                    <td><i class="success fa fa-clock-o" title="Ngày cập nhật"></i> {{$value->updated_at}}</td>
                    <td class="last"> <a href="/admin/tag/edit/{{$value->id}}"><i class="success fa fa-edit"></i> Sửa</a> | <a href="/admin/tag/del/{{$value->id}}"><i class="success fa fa-remove"></i> Xóa</a> </td>
                  </tr>
                  @endforeach
                </tbody>
              </table>
              <div class="btn-toolbar pull-right">
                  {{$listTag->appends($conditionPage)->links()}}
              </div>
            </div>
          </div>
        </div>
