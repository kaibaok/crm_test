            ...
            <div class="x_panel">
              <div class="x_content autocomp">
                    ...
                    <div class="item form-group  @if(isset($errors['number'])) bad @endif">
                        ...
                        <div class="col-md-4 col-sm-4 col-xs-12">
                        <input type="text" id="number" class="form-control col-md-7 col-xs-12" name="number" value="{{isset($cartDetail['number']) ? $cartDetail['number'] : 1 }}">
                        </div>
                        ...
                        <div class="col-md-4 col-sm-4 col-xs-12">
                            <select id="size" class="form-control col-md-7 col-xs-12" name="size">
                                @foreach($listSize as $key => $value)
                                    @if($value)
                                        @php $select = ($cartDetail['size'] == $value) ? "selected" : ""; @endphp
                                        <option value="{{$value}}" {{ $select }}> {{ strtoupper($value) }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
