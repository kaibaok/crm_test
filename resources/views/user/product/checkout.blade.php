                    ...
                    <div class="billing-detail">
                        ...
                        <form action="" method="post">
                            ...
                            <p>Họ và tên <span>*</span><br>
                                <input type="text" placeholder="Họ và tên" name="full_name" value="{{!empty($info['full_name']) ? $info['full_name'] : ''}}">
                                @if(isset($errors['full_name']))
                                <span class="txt_error">{{$errors['full_name']}}</span>
                                @endif
                            </p>
                            <div class="row">
                                <p class="col-sm-6 col-xs-12">
                                    Email <span>*</span><br>
                                    <input type="email" placeholder="Email" name="email" value="{{!empty($info['email']) ? $info['email'] : ''}}">
                                    @if (isset($errors['email']))
                                    <span class="txt_error">{{$errors['email']}}</span>
                                    @endif
                                </p>
                                <p class="col-sm-6 col-xs-12">
                                    Số điện thoại <span>*</span><br>
                                    <input type="text" placeholder="Số điện thoại" name="phone" value="{{!empty($info['phone']) ? $info['phone'] : ''}}">
                                    @if (isset($errors['phone']))
                                    <span class="txt_error">{{$errors['phone']}}</span>
                                    @endif
                                </p>
                            </div>
                            ...
                            <input type="text" placeholder="Địa chỉ 2" name="address2" value="{{!empty($info['address2']) ? $info['address2'] : ''}}">
                            @if(isset($errors['address']))
                            <span class="txt_error">{{$errors['address']}}</span>
                            @endif
                            </p>
                            ...
                            <div class="row">
                                <p class="col-sm-6 col-xs-12">
                                    Phường/Xã <br>
                                    <input type="text" placeholder="Phường/Xã" name="stage" value="{{!empty($info['stage']) ? $info['stage'] : ''}}">
                                    @if(isset($errors['stage']))
                                    <span class="txt_error">{{$errors['stage']}}</span>
                                    @endif
                                </p>
                                <p class="col-sm-6 col-xs-12">
                                    Quận <span>*</span><br>
                                    <input type="text" placeholder="Quận" name="district" value="{{!empty($info['district']) ? $info['district'] : ''}}">
                                    @if(isset($errors['district']))
                                    <span class="txt_error">{{$errors['district']}}</span>
                                    @endif
                                </p>
                            </div>
                            <div class="row">
                                    ...
                                    <ul>
                                        <li>
                                            ...
                                            @php $totalPrice += $value['price'] * $value['number']; @endphp
                                                <p>{{$value['title']}}    <span> x  {{$value['number']}}</span><span>{{$value['price'] * $value['number']}}</span></p>
                                            @endforeach
